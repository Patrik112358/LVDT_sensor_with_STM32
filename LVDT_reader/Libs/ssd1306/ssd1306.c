...
void ssd1306_FillCircle(uint8_t par_x, uint8_t par_y, uint8_t par_r, SSD1306_COLOR par_color)
{
  ...
  do {
    for (uint8_t _y = (par_y + y); _y >= (par_y - y); _y--) {
      for (uint8_t _x = (par_x - x); _x >= (par_x + x); _x--) { ssd1306_DrawPixel(_x, _y, par_color); }
    }
...
void ssd1306_FillRectangle(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2, SSD1306_COLOR color)
{
  ...
  for (uint8_t y = y_start; (y <= y_end) && (y < SSD1306_HEIGHT); y++) {
    for (uint8_t x = x_start; (x <= x_end) && (x < SSD1306_WIDTH); x++) { ssd1306_DrawPixel(x, y, color); }
  }
  return;
}
...
SSD1306_Error_t ssd1306_InvertRectangle(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2)
{
  ...
  if ((y1 / 8) != (y2 / 8)) {
    ...
    for (uint32_t x = x1; x <= x2; x++) {
      ...
      i += SSD1306_WIDTH;
      for (; i < x + (y2 / 8) * SSD1306_WIDTH; i += SSD1306_WIDTH) { SSD1306_Buffer[i] ^= 0xFF; }
