    ...
    float target_frequency)
{
  ...
  for (uint32_t i = 0; i < data_size; i++) {
    ...
    float    adc1_scaled = __LL_ADC_CALC_DATA_TO_VOLTAGE(VDDA_APPLI, adc1, LL_ADC_RESOLUTION_12B);
    float    adc2_scaled = __LL_ADC_CALC_DATA_TO_VOLTAGE(VDDA_APPLI, adc2, LL_ADC_RESOLUTION_12B);
